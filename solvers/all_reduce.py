...
class Solver(DistributedMPISolver):
    ...
    def worker_run(cls, n_iter, X_local, args, comm, rank, world_size):
        ...
        for _ in range(n_iter):
            ...
            logs['sample_time'].append(time.time() - t_start)

            # 2. Local Computation
            t_start = time.time()
            np.matmul(X_batch.T, X_batch @ W, out=G_local)
            logs['compute_time'].append(time.time() - t_start)

            # 3. Communication
            t_start = time.time()
            comm.Allreduce(G_local, G_global, op=MPI.SUM)
            G_global /= args.batch_size
            logs['comm_time'].append(time.time() - t_start)

            # Global Update
            t_start = time.time()
            b = np.sqrt(b**2 + np.linalg.vector_norm(G_global, axis=0) ** 2)
            W += G_global / b[None, :]
            logs['update_time'].append(time.time() - t_start)
            ...
            signs[signs == 0] = 1.0
            W *= signs[None, :]
            logs['ortho_time'].append(time.time() - t_start)
